...
st.markdown(f"""
<div style="background-color:#1e1e1e; padding:20px; border-radius:12px; color:#f5f5f7; margin-top:20px;">
    <h4 style="color:#ff4757;">📊 Stock Summary</h4>
    <p><strong>Name:</strong> {info.get('shortName', 'N/A')}</p>
    <p><strong>Market Cap:</strong> ₹ {info.get('marketCap', 'N/A'):,}</p>
    <p><strong>Open:</strong> ₹ {info.get('open', 'N/A')}</p>
    ...
</div>
""", unsafe_allow_html=True)
